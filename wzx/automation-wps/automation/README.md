...
代码不再依赖 `/home/lzx`，用户主目录统一读取 Ubuntu 当前桌面用户的 `$HOME`。

## 1. 项目目录

建议把项目放到 Ubuntu 当前用户的主目录：

```bash
cd "$HOME"
mkdir -p automation-wps
```
...
```bash
mkdir -p "$HOME/automation-wps/samples/wps"
mkdir -p "$HOME/automation-wps/outputs/wps"
```
