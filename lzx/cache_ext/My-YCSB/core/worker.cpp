...
void worker_thread_fn(Client *client, Workload *workload, OpMeasurement *measurement, long next_op_interval_ns) {
	...
	while (workload->has_next_op()) {
		...
		try {
			...
			// The workload can run out between has_next_op() and
			// next_op(): trace files only set eofbit after a failed
			// read, and another thread may consume the last op.
			// Treat exhaustion as a clean end of the run.
			fprintf(stderr, "worker %d: trace exhausted, ending run\n", client->id);
			break;
		}
