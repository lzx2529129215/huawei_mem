...
#include "page_cache_ext_mglru.skel.h"
#include "dir_watcher.h"

char *USAGE = "Usage: ./page_cache_ext_mglru\n";
struct cmdline_args {
	char *watch_dir;
};
...
{
	int ret;
	struct page_cache_ext_mglru_bpf *skel;
	...
	// Open skel
	skel = page_cache_ext_mglru_bpf__open();
	if (skel == NULL) {
	...
	// Load programs
	ret = page_cache_ext_mglru_bpf__load(skel);
	if (ret) {
		fprintf(stderr, "Failed to load BPF skeleton: %s\n",
			strerror(errno));
		page_cache_ext_mglru_bpf__destroy(skel);
		return 1;
	}
	...
	// Attach probes
	ret = page_cache_ext_mglru_bpf__attach(skel);
	if (ret) {
