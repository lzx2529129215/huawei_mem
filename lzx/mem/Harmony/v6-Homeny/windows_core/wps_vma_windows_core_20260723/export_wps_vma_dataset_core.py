#!/usr/bin/env python3
"""Export a compact, quality-filtered WPS VMA dataset.
...
def _catalog_labels(root: Path, explicit: Path | None) -> list[str]:
    ...
    candidates.extend((root / "operation_catalog.json", root / "wps_operation_catalog.json"))
...
def export_dataset(args: argparse.Namespace) -> dict:
    ...
    if not expected_labels and args.expected_label_count < 1:
        raise ValueError("找不到 operation catalog；请使用 --expected-label-count 18 或 --catalog 指定目录")
    ...
    output_dir = args.output_dir.expanduser().resolve() if args.output_dir else root / f"wps_vma_dataset_core_{timestamp}"
    ...
    summary = {
        "schema_version": "wps.vma-dataset-core.v1",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--root", type=Path, required=True, help="Windows WPS 采集输出根目录")
    ...
    parser.add_argument("--expected-label-count", type=int, default=18, help="无 catalog 时每个 trial 期望的操作数量")
...
if __name__ == "__main__":
    try:
        ...
        print(f"[wps-core-export] ERROR: {exc}", file=sys.stderr)
        raise SystemExit(2)
