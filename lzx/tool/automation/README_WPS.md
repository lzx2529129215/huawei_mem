# lzx/tool/automation 中的 WPS 自动化
...
执行器不依赖固定用户主目录；项目根目录由脚本所在的 `lzx/tool/automation`
自动推导，场景位于 `lzx/tool/configs/automation`，样本位于 `lzx/tool/samples/wps`。
...
```bash
cd /home/lzxxxxxx/桌面/huawei/myself-kswapd/lzx/tool
