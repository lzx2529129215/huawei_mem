...
class PipelineAuditHelpersTest(unittest.TestCase):
    ...
    def test_mapping_keeps_model_and_runtime_spaces(self):
        config = json.loads((ROOT / "configs/runtime/test1_runtime_app_scope.json").read_text(encoding="utf-8"))  #lzx
        vocab = json.loads((ROOT / "operation_predictor/data/vocab/test1/app_vocab_duration.json").read_text(encoding="utf-8"))  #lzx
        apps = {item["app_key"]: item for item in config["apps"]}
        self.assertEqual(vocab[apps["FIREFOX"]["vocab_name"]], 0)  #lzx
        self.assertEqual(apps["FIREFOX"]["app_id"], 1)  #lzx
        self.assertNotEqual(  #lzx
            vocab[apps["FIREFOX"]["vocab_name"]], apps["FIREFOX"]["app_id"])  #lzx
