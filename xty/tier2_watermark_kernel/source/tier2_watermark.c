 ...
 */
/* Forward declaration */
static int __init tier2_memcg_register_files(void);
...
/* Forward declaration for per-memcg cgroup file registration */
// SPDX-License-Identifier: GPL-2.0
/*
 * Per-memcg tier2 watermark additions to tier2_watermark.c
 *
 * This file contains the ADDITIONAL code to be appended to the existing
 * mm/tier2_watermark.c to add per-memcg (per-cgroup) tier2 watermark support.
 *
 * Watermark calculation for per-memcg:
 *   headroom = limit_bytes - current_usage_bytes
 *   alloc_wmark_bytes = max(PAGE_SIZE, limit_bytes * alloc_scale / 10000)
 *   demote_wmark_bytes = max(alloc_wmark_bytes, limit_bytes * demote_scale / 10000)
 *   below_alloc  = (headroom < alloc_wmark_bytes)
 *   below_demote = (headroom < demote_wmark_bytes)
 *
 * If limit is unlimited, per-memcg tier2 is effectively disabled.
 */

#include <linux/memcontrol.h>
#include <linux/page_counter.h>
#include <linux/cgroup.h>
#include "../../include/linux/tier2_watermark.h"
#include "internal.h"

/*
 * Work around: include the actual header path as used in kernel build.
 * The header is at include/linux/tier2_watermark.h
 */
#ifndef CONFIG_TIER2_WATERMARK
#error "This file requires CONFIG_TIER2_WATERMARK=y"
#endif
...
/*
 * Register per-memcg tier2 cgroup files with the memory subsystem.
 * Called from tier2_wmark_init().
 */
static int __init tier2_memcg_register_files(void)
{
	...
	/* Find the memory cgroup subsystem */
	ss = &memory_cgrp_subsys;
	if (!ss || !ss->legacy_cftypes) {
		pr_warn("tier2_watermark: memory cgroup subsystem not ready, per-memcg files skipped\n");
		return -ENODEV;
	}

	ret = cgroup_add_legacy_cftypes(ss, tier2_memcg_files);
	if (ret && ret != -EBUSY)
		pr_warn("tier2_watermark: failed to register per-memcg cgroup files: %d\n", ret);
	else
		pr_info("tier2_watermark: per-memcg cgroup files registered\n");

	return ret;
}

/* Keep the existing #include and CONFIG_SYSCTL blocks below this point */
