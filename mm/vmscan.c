	...
	if (actual_tier_protect) {
		...
		if (policy_upgrade) {
			if (!parp_policy_inc_gen(lruvec, folio, &gen)) {
				int raced_gen = folio_lru_gen(folio);

				tier_decision->delta_tier_q8 = 0;
				tier_decision->effective_tier_q8 =
					tier * PARP_TIER_SCALE;
				tier_decision->effective_protect =
					tier_decision->native_protect;
				tier_decision->actual_tier_protect =
					tier_decision->native_protect;
				tier_decision->action = tier > tier_idx ?
					PARP_TIER_KEEP_PROTECT :
					PARP_TIER_KEEP_RECLAIM;
				tier_decision->bypass =
			...
			struct scan_control *sc, int swappiness)
{
	...
	parp_effective_tier_lock_start(&parp_lock);
	spin_lock_irq(&lruvec->lru_lock);
	parp_effective_tier_lock_acquired(&parp_lock);
	...
	parp_effective_tier_lock_releasing(&parp_lock);
	spin_unlock_irq(&lruvec->lru_lock);
	parp_effective_tier_lock_released(&parp_lock,
...
retry:
	...
	parp_effective_tier_feedback(lruvec, type, isolated, reclaimed);
